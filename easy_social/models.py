...
class Post(db.Model):
    ...
    post_type = db.Column(db.String(20), nullable=False, default=POST_TYPE_STANDARD, index=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "(post_type = 'poll') OR (length(body) > 0) OR (media_filename IS NOT NULL) "
    ...
    def is_poll(self) -> bool:
        return self.post_type == self.POST_TYPE_POLL and self.poll is not None
...
class PollOption(db.Model):
    ...
    __table_args__ = (
        CheckConstraint("position >= 0 AND position < 4", name="ck_poll_option_position"),
...
class PollVote(db.Model):
    ...
    __table_args__ = (UniqueConstraint("poll_id", "user_id", name="uq_poll_vote_once"),)
